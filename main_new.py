...
from pyspark.sql.functions import col, lit, min as spark_min, max as spark_max, count
...
bed_files_path_train = "./bed_files_train.txt"
bed_files_path_test = "./bed_files_test.txt"
# motif_file1 = "./ref_files/allreads_v7_0_out5p.csv"
# motif_file2 = "./ref_files/allreads_v7_0_in5p.csv"
# motif_file3 = "./ref_files/MSK_allreads_v7_0_length_LLH.csv"
metadata_file = "./ref_files/info_ctDNA.txt"
...
def data_prep(column_names,
              bed_file,
             sample_labels):
    
    df = spark.read.option("delimiter", "\t").csv(bed_file).sample(fraction=0.01, seed=49)
    for i, col_name in enumerate(column_names[:len(df.columns)]):
            df = df.withColumnRenamed(f"_c{i}", col_name)
    df = df.drop("chromosome", "length_bin", "gc_weight", "is_shifted", "cnv_weight")
    # df = df.withColumn("length_bin", regexp_replace(col("length_bin"), "-", "_"))

    # Compute fragment_length
    df = df.withColumn("fragment_length", col("frag_end") - col("frag_start"))

    # Frequency encoding for out5p and in5p
    for col_name in ["out5p", "in5p"]:
        freq_df = df.groupBy(col_name).agg(count(lit(1)).alias(f"{col_name}_freq"))
    ...
    df = df.drop("in5p", "out5p", "chromosome", "frag_start", "frag_end", "sample_name")
    # Normalize
    normalize_cols = ["fragment_length", "out5p_freq", "in5p_freq"]
...
def create_dataloader(bed_files, batch_size=1, shuffle=True):
    ...
    for bed_file in bed_files:
        ...
        processed_df, sample_label = data_prep(column_names, bed_file, sample_labels)
...
def plot_training_metrics(train_losses, train_aucs, save_path="/n/data1/hms/dbmi/gulhan/lab/ankit/scripts/MIL/plots/training_metrics_new.png"):
    """
...
save_path = "/n/data1/hms/dbmi/gulhan/lab/ankit/scripts/MIL/models/trained_model_new.pth"
...
plot_attention_scores(test_attention_scores, sample_idx=0, save_path="/n/data1/hms/dbmi/gulhan/lab/ankit/scripts/MIL/plots/attention_scores_sample_0_new.png")
...
plot_attention_distribution(test_attention_scores, true_test_labels, save_path="/n/data1/hms/dbmi/gulhan/lab/ankit/scripts/MIL/plots/attention_distribution_new.png")
