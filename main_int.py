...
from pyspark.sql.functions import col, regexp_replace, lit, regexp_extract, when, trim, min as spark_min, max as spark_max

from dataloader_int import MILDataset
...
def data_prep(motif_file1,
    ...
    df = spark.read.option("delimiter", "\t").csv(bed_file).sample(fraction=0.1, seed=42)
    ...
    # Iterate through all columns and clean them, convert to float and normalize
    for col_name in joined_df3.columns:
        ...
        joined_df3 = joined_df3.withColumn(col_name, col(col_name).cast("float"))
    
        # normalize
        min_val = joined_df3.select(spark_min(col(col_name))).collect()[0][0]
        max_val = joined_df3.select(spark_max(col(col_name))).collect()[0][0]

        # Avoid division by zero in case max == min
        if min_val != max_val:
            joined_df3 = joined_df3.withColumn(col_name, (col(col_name) - min_val) / (max_val - min_val))
        else:
            joined_df3 = joined_df3.withColumn(col_name, col(col_name) * 0)
...
def plot_training_metrics(train_losses, train_aucs, save_path="/n/data1/hms/dbmi/gulhan/lab/ankit/scripts/MIL/plots/training_metrics_int_new.png"):
    """
...
save_path = "/n/data1/hms/dbmi/gulhan/lab/ankit/scripts/MIL/models/trained_model_int_new.pth"
...
plot_attention_scores(test_attention_scores, sample_idx=0, save_path="/n/data1/hms/dbmi/gulhan/lab/ankit/scripts/MIL/plots/attention_scores_sample_0_int_new.png")
...
plot_attention_distribution(test_attention_scores, true_test_labels, save_path="/n/data1/hms/dbmi/gulhan/lab/ankit/scripts/MIL/plots/attention_distribution_int_new.png")
